...
.PHONY: help fmt tidy tidy-check test test-race coverage vet build run lint vuln gosec sqlc-diff audit check up down install-cert
...
help:
	@printf "%s\n" \
		...
		"make up           - start the local compose stack (generates TLS certs on first run)" \
