...
# returns 200 or the timeout expires. Used by endpoint-driven functional and
# integration test harnesses before running tests.
set -euo pipefail
...
echo "Check the service manager logs for the deployment under test (snap, Juju, or local process)." >&2
exit 1
