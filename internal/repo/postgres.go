...
CREATE TABLE IF NOT EXISTS schema_migrations (
	...
	defer func() {
		// Advisory lock release is best-effort; connection close also releases it.
		_ = p.pool.QueryRow(ctx, "SELECT pg_advisory_unlock($1, $2)", lockClass, lockObjID).Scan(nil)
	}()
